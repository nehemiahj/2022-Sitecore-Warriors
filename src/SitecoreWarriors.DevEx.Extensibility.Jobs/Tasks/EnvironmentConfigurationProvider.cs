...
namespace SitecoreWarriors.DevEx.Extensibility.Jobs.Tasks
{
    public class EnvironmentConfigurationProvider : IEnvironmentConfigurationProvider
    {
        ...
        public EnvironmentConfigurationProvider(IRootConfigurationManager rootConfigurationManager) => _rootConfigurationManager = rootConfigurationManager;
          ...
          string environmentName)
        {
            ...
            if (!(await _rootConfigurationManager.ResolveRootConfiguration(currentDirectory)).Environments.TryGetValue(environmentName, out configurationAsync))
